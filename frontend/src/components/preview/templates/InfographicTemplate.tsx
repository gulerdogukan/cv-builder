...
export default function InfographicTemplate({ data }: Props) {
  ...
  return (
    <div className="min-h-[297mm] p-0 font-sans text-slate-800 bg-white max-w-4xl mx-auto overflow-hidden relative" style={{ fontFamily: 'var(--cv-font)' }}>
      {/* Heavy Graphic Header */}
      <header className="p-12 relative overflow-hidden text-center pb-20" style={{ backgroundColor: 'var(--cv-accent)', color: 'white' }}>
        <div className="absolute -top-24 -left-24 w-64 h-64 bg-black/10 rounded-full mix-blend-multiply opacity-50 blur-2xl"></div>
        <div className="absolute -bottom-24 -right-24 w-80 h-80 bg-black/10 rounded-full mix-blend-multiply opacity-50 blur-2xl"></div>
        
        <div className="relative z-10 w-full">
          <h1 className="text-5xl font-black tracking-tight mb-4 uppercase">{personal.fullName || 'Ad Soyad'}</h1>
          <h2 className="text-2xl font-light tracking-widest uppercase mb-8" style={{ opacity: 0.8 }}>{personal.profession || 'Vizyoner Uzman'}</h2>
          
          <div className="flex flex-wrap items-center justify-center gap-6 text-sm font-medium">
            {personal.email && <div className="bg-white/10 px-4 py-2 rounded-full backdrop-blur-sm border border-white/20">E: {personal.email}</div>}
            {personal.phone && <div className="bg-white/10 px-4 py-2 rounded-full backdrop-blur-sm border border-white/20">T: {personal.phone}</div>}
            {personal.location && <div className="bg-white/10 px-4 py-2 rounded-full backdrop-blur-sm border border-white/20">Konum: {personal.location}</div>}
            {personal.linkedin && <div className="bg-white/10 px-4 py-2 rounded-full backdrop-blur-sm border border-white/20">IN: {personal.linkedin}</div>}
          </div>
        </div>
      </header>
      ...
      <div className="px-10 -mt-10 relative z-20 space-y-12">
        {/* Summary Card */}
        {summary && (
          <div className="bg-white rounded-2xl shadow-xl shadow-slate-900/5 border border-slate-100 p-8 text-center mx-auto max-w-3xl">
            <h3 className="text-xs font-black uppercase tracking-[0.2em] mb-4" style={{ color: 'var(--cv-accent)' }}>Profil</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-4 pb-12">
                ...
                <div className="space-y-0 text-center relative before:absolute before:left-1/2 before:top-0 before:bottom-0 before:w-1 before:bg-slate-100 before:-translate-x-1/2">
                  {experience.map((exp) => (
                    <div key={exp.id} className="relative py-6 z-10 w-full flex flex-col items-center">
                      <div className="w-16 h-1 mb-4 rounded-full shadow-sm relative z-20 ring-4 ring-white" style={{ backgroundColor: 'var(--cv-accent)' }}></div>
                      <div className="bg-white border border-slate-200 shadow-sm p-4 w-full rounded-2xl relative z-10 box-border text-center">
                        <div className="text-xs font-bold tracking-widest mb-2 uppercase" style={{ color: 'var(--cv-accent)' }}>{exp.startDate} — {exp.isCurrent ? 'Günümüz' : exp.endDate}</div>
                ...
                <div className="space-y-6">
                  {education.map(edu => (
                    <div key={edu.id} className="text-center relative pb-6 last:pb-0 border-b border-slate-200 last:border-0 border-dashed">
                      ...
                      <div className="text-sm font-semibold mb-2" style={{ color: 'var(--cv-accent)' }}>{edu.degree} - {edu.field}</div>
          ...
          <div className="space-y-12">
                    ...
                    return (
                      <div key={s.id} className="relative">
                        <div className="flex justify-between items-end mb-1">
                          ...
                          <span className="text-[10px] font-black tracking-widest uppercase" style={{ color: 'var(--cv-accent)' }}>{s.level}</span>
                        </div>
                        <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: widthMap[s.level] || '50%', backgroundColor: 'var(--cv-accent)' }}></div>
                        </div>
                      </div>
                    )
                  })}
            ...
            {languages.length > 0 && (
              <section className="rounded-3xl p-8 border border-slate-100" style={{ backgroundColor: 'rgba(0,0,0,0.02)' }}>
                <h3 className="text-sm font-black text-center text-slate-900 uppercase tracking-[0.2em] mb-6">Poliglot Profil</h3>
                <div className="grid grid-cols-2 gap-4">
                  {languages.map(l => (
                    <div key={l.id} className="bg-white p-4 rounded-2xl text-center shadow-sm border border-slate-100 border-b-4 border-b-slate-200">
                      <div className="font-black text-slate-800 mb-1 leading-snug">{l.name}</div>
                      <div className="text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--cv-accent)' }}>{l.level}</div>
                    </div>
                  ))}
                </div>
              </section>
                ...
                <div className="space-y-3">
                  {certifications.map(c => (
                    <div key={c.id} className="flex gap-4 items-center bg-white border border-slate-200 p-4 rounded-xl shadow-sm">
                      <div className="w-10 h-10 shrink-0 bg-slate-100 rounded-full flex items-center justify-center font-black text-xl" style={{ color: 'var(--cv-accent)' }}>
                        ★
                      </div>
                      <div className="flex-1">
                        ...
                        <div className="text-xs font-medium text-slate-500 mt-1 flex justify-between">
                          <span>{c.issuer}</span>
                          <span className="font-bold" style={{ color: 'var(--cv-accent)' }}>{c.date}</span>
                        </div>
                      </div>
                    </div>
                  ))}
