...
export default function MinimalistTemplate({ data }: Props) {
  ...
  return (
    <div className="p-10 font-serif text-gray-900 leading-relaxed max-w-4xl mx-auto">
      {/* Header */}
      <header className="text-center mb-8">
        <h1 className="text-3xl font-bold uppercase tracking-wider mb-2">{personal.fullName || 'AD SOYAD'}</h1>
        <div className="text-sm flex flex-wrap justify-center gap-x-4 gap-y-1 text-gray-700">
          {personal.email && <span>{personal.email}</span>}
          {personal.phone && <span>{personal.phone}</span>}
          {personal.location && <span>{personal.location}</span>}
          {personal.linkedin && <a href={`https://${personal.linkedin}`} className="text-blue-800 hover:underline">{personal.linkedin}</a>}
          {personal.github && <a href={`https://${personal.github}`} className="text-blue-800 hover:underline">{personal.github}</a>}
        </div>
      </header>
      ...
      {summary && (
        <section className="mb-6">
          <h2 className="text-lg font-bold uppercase border-b-2 border-gray-800 mb-2">Profesyonel Özet</h2>
      ...
      {experience.length > 0 && (
        <section className="mb-6">
          <h2 className="text-lg font-bold uppercase border-b-2 border-gray-800 mb-3">Deneyim</h2>
      ...
      {education.length > 0 && (
        <section className="mb-6">
          <h2 className="text-lg font-bold uppercase border-b-2 border-gray-800 mb-3">Eğitim</h2>
          <div className="space-y-4">
            {education.map((edu) => (
              <div key={edu.id}>
                <div className="flex justify-between items-baseline mb-1">
      ...
      <div className="grid grid-cols-2 gap-6">
        {skills.length > 0 && (
          <section>
            <h2 className="text-lg font-bold uppercase border-b-2 border-gray-800 mb-3">Beceriler</h2>
        ...
        <div>
          {languages.length > 0 && (
            <section className="mb-6">
              <h2 className="text-lg font-bold uppercase border-b-2 border-gray-800 mb-3">Diller</h2>
          ...
          {certifications.length > 0 && (
            <section>
              <h2 className="text-lg font-bold uppercase border-b-2 border-gray-800 mb-3">Sertifikalar</h2>
