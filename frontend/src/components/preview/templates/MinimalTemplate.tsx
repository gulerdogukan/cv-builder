...
export default function MinimalTemplate({ data }: Props) {
  ...
  return (
    <div className="bg-white text-gray-800 px-10 py-8" style={{ fontFamily: 'Helvetica Neue, Helvetica, Arial, sans-serif', fontSize: '11px', lineHeight: '1.6' }}>
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-[20px] font-light tracking-tight text-gray-900">{personal.fullName || 'Ad Soyad'}</h1>
      ...
      <div className="space-y-5">
        ...
        {experience.length > 0 && (
          <section>
            <div className="text-[8px] font-semibold uppercase tracking-[3px] text-gray-400 mb-2">Deneyim</div>
        ...
        {education.length > 0 && (
          <section>
            <div className="text-[8px] font-semibold uppercase tracking-[3px] text-gray-400 mb-2">Eğitim</div>
        ...
        <div className="grid grid-cols-2 gap-5">
          {skills.length > 0 && (
            <section>
              <div className="text-[8px] font-semibold uppercase tracking-[3px] text-gray-400 mb-2">Beceriler</div>
              <div className="flex flex-wrap gap-1">
                {skills.map(s => (
                  <span key={s.id} className="text-[9px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{s.name}</span>
                ))}
              </div>
            </section>
          )}
          {languages.length > 0 && (
            <section>
              <div className="text-[8px] font-semibold uppercase tracking-[3px] text-gray-400 mb-2">Diller</div>
        ...
        {certifications.length > 0 && (
          <section>
            <div className="text-[8px] font-semibold uppercase tracking-[3px] text-gray-400 mb-2">Sertifikalar</div>
