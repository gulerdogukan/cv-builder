...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Supabase email confirm açıksa kullanıcı doğrulama yapmalı
      // Kapalıysa otomatik giriş yapar → isAuthenticated tetiklenir
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted/50 px-4 py-8">
      <div className="w-full max-w-md rounded-xl border bg-card p-8 shadow-sm">
        <div className="text-center mb-8">
          ...
          <p className="mt-3 text-sm text-muted-foreground">Ücretsiz hesap oluşturun</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="fullName" className="block text-sm font-medium mb-1.5">Ad Soyad</label>
            <input
              id="fullName"
              type="text"
              value={fullName}
